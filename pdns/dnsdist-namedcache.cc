...
// lookup query in cache -
// It walks the name right to left after second position and does a lookup on the resulting domain.
//
//  For example   fee.fi.fo.fum.com  would be have lookups for
//             fum.com
//             fo.fum.com
//             fi.fo.fum.com
//             fee.fi.fo.fum.com
//
//   The content string of lookup is placed in strData.
//   The function returns the cache status
...
int iLoc = CACHE_HIT::HIT_NONE;
    ...
    bool bDebug = true;        // force debug
      ...
      if (bDebug == true)
        warnlog("DEBUG - DEBUG - DEBUG - lookupWild() - length......: %d    strQuery: %s \n", iSize, strQuery.c_str());
      for(int ii=iSize-1; ii > 0; ii--) {
          if(strQuery.at(ii) == '.') {
           ...
           if (bDebug == true)
             warnlog("DEBUG - DEBUG - DEBUG - lookupWild() - -----> ii: %d   testName: %s \n", ii, strTestName.c_str());
           iLoc = lookup(strTestName, strData);
           if(iLoc != CACHE_HIT::HIT_NONE) {
             if (bDebug == true)
               warnlog("DEBUG - DEBUG - DEBUG - lookupWild() - GOT MATCH   testName: %s \n", strTestName.c_str());
             return iLoc;
           }
         }
      }
      if (bDebug == true)
        warnlog("DEBUG - DEBUG - DEBUG - lookupWild() - -----> last lookup: %s \n", strQuery.c_str());
